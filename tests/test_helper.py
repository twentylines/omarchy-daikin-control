...
class HelperTests(unittest.TestCase):
    ...
    def test_keyboard_shortcuts_persist_and_generate_reversible_hyprland_bindings(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertIn("call sai.homeassistant-ac open_panel", bindings)
                self.assertIn("hl.dsp.exec_cmd", bindings)
                self.assertIn('pgrep -xo quickshell', bindings)
                self.assertNotIn("hl.dsp.global", bindings)
                self.assertIn("SUPER + ALT + P + 1", bindings)
                self.assertIn("call sai.homeassistant-ac power_2", bindings)
    ...
    def test_reset_appearance_restores_defaults_without_disabling_customisations(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertTrue(parsed["appearance_outer_border_enabled"])
                saved = json.loads(helper.CONFIG_PATH.read_text(encoding="utf-8"))
    ...
    def test_config_file_mode_saves_preferences_without_exposing_token(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertTrue(saved["appearance_outer_border_enabled"])
                self.assertTrue(saved["global_tab_navigation_enabled"])
    ...
    def test_temperature_units_and_chart_gradient_are_wired(self):
        ...
        self.assertIn('text: "CTRL+S / CTRL+ENTER"', panel)
        self.assertIn('event.key === Qt.Key_S', panel)
        self.assertIn('event.key === Qt.Key_R', panel)
        self.assertIn('id: configFileApplyButton', panel)
        self.assertIn('focusable: true', panel)
        self.assertIn('configFileApplyButton.forceActiveFocus()', panel)
        self.assertIn('configFileReloadButton.forceActiveFocus()', panel)
        self.assertIn('command = ["python3", root.helperPath, "set-config-file"]', panel)
        self.assertIn("Ctrl+Enter", panel)
        self.assertIn('label: "Compact UI · flatten inner cards"', panel)
        self.assertIn('label: "Outer panel border"', panel)
        self.assertIn("appearance_outer_border_enabled", panel)
        ...
        self.assertIn('EXTERNAL LOG STALE · LAST WINDOW UNAVAILABLE', panel)
        ...
        self.assertIn("historyChartPoints", panel)
        self.assertIn("historyChartWindowEnd", panel)
        ...
        self.assertIn('property string globalTabFocusOverrideMode: ""', panel)
        self.assertIn("function restoreGlobalTabOverrides()", panel)
        self.assertIn('item.activeFocusOnTab = false', panel)
        self.assertIn('activeFocusOnTab: false', panel)
        self.assertIn("if (!root.configFileModeEnabled", panel)
        self.assertIn("openSettingsShortcutDisplay", panel)
        self.assertIn("openSettingsShortcutActive", panel)
        self.assertIn("function focusConfirmationItem(item)", panel)
        self.assertIn("function focusReplacementInPanelBranch(item, items, direction)", panel)
        self.assertIn("root.focusReplacementInPanelBranch(focusedItem, items, direction)", panel)
        self.assertIn("onResetAppConfirmingChanged", panel)
        self.assertIn("onUninstallConfirmingChanged", panel)
        self.assertIn("root.focusConfirmationItem(uninstallOptionConfirmButton)", panel)
        self.assertIn("root.focusNextPanelItem(", panel)
        ...
        self.assertIn("windowEndTimestamp: root.historyChartWindowEnd", panel)
        self.assertIn("function formatHistoryDuration(value)", panel)
        self.assertIn('text: "READINGS FROM THE LAST "', panel)
        self.assertIn("root.formatHistoryDuration(root.historyHours)", panel)
        self.assertIn('text: root.formatHours(root.rangeHours)', chart)
        self.assertNotIn('text: root.formatHours(root.rangeHours) + " H"', chart)
        self.assertNotIn("PAST ", chart)
        ...
        self.assertIn("root.setupAddressPort", panel)
        self.assertIn("id: setupPortField", panel)
        self.assertIn("id: reconnectPortField", panel)
        self.assertIn("root.globalTabNavigationEnabled", panel)
        self.assertIn("root.latencyColor(root.remoteHistoryPingMs", panel)
        self.assertIn("editorBorderSpec", panel)
        self.assertIn("property real windowEndTimestamp: 0", chart)
        self.assertIn("root.windowEndTimestamp > 0", chart)
        critical = (HELPER.parent / "CriticalActionSplit.qml").read_text(encoding="utf-8")
    ...
    def test_0_3_4_chart_and_surface_regressions_are_guarded(self):
        panel = (HELPER.parent / "Panel.qml").read_text(encoding="utf-8")
        chart = (HELPER.parent / "TemperatureHistoryChart.qml").read_text(encoding="utf-8")
        status = (HELPER.parent / "ServerStatus.qml").read_text(encoding="utf-8")
        self.assertIn('sourceLabel: root.historySourceLabel', panel)
        self.assertNotIn('sourceLabel: root.historySourceLabel + (root.historyLogStale', panel)
        self.assertIn('color: root.accentColor', panel)
        self.assertIn('warningColor: root.warning', panel)
        self.assertIn('anchors.leftMargin: -panel.padding', panel)
        self.assertNotIn('anchors.leftMargin: -(panel.padding + Border.left(panel.borderSpec))', panel)
        self.assertIn('readonly property real editorRightInset: Style.space(18)', panel)
        self.assertIn('width: Math.max(1, configFileEditorScroll.width', panel)
